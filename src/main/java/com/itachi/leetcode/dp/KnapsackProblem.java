...
public class KnapsackProblem {
    ...
    public static void main(String[] args) {
        ...
        // 注意！这个table按照约定，第一行和第一列为0
        // 因此table的行数 = 物品的数量 + 1 ，列数 = maxWeight+1
        int[][] table = new int[weight.length + 1][maxWeight + 1];

        // 动态规划处理table这个二维数组
        for (int i = 1; i < table.length; i++) {
            for (int j = 1; j < table[i].length; j++) {
                //因为table的行数 = 物品的数量 + 1，所以此处 i-1
                //放入物品的重量大于了 j，那么直接copy上一层的价值
                if (weight[i - 1] > j) {
                    table[i][j] = table[i - 1][j];
                } else {
                    //比较上一层和加入这个物品后 + 满足剩余容量的价值，两者取最大值
 ...
 *
 * 作者：星晴pro
 * 链接：https://leetcode.cn/problems/last-stone-weight-ii/solutions/805162/yi-pian-wen-zhang-chi-tou-bei-bao-wen-ti-5lfv/
 * 来源：力扣（LeetCode）
